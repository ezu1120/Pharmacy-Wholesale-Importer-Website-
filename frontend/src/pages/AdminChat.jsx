...
export default function AdminChat() {
  ...
  return (
    <AdminLayout title={selectedChat && typeof window !== 'undefined' && window.innerWidth < 768 ? null : "Chat Portal"} subtitle={selectedChat && typeof window !== 'undefined' && window.innerWidth < 768 ? null : "Manage your real-time customer support inquiries."}>
      <div className={`
        flex border border-gray-200 bg-white rounded-2xl shadow-sm overflow-hidden font-body relative
        ${selectedChat && typeof window !== 'undefined' && window.innerWidth < 768 ? 'h-[calc(100dvh-120px)] -mt-10' : 'h-[calc(100vh-220px)]'}
      ...
      `}>
        ...
        `}>
          <div className="h-16 px-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50 flex-shrink-0">
            <h3 className="text-[11px] font-black text-gray-900 uppercase tracking-[0.15em] flex items-center gap-2">
              Inbox
              <span className="bg-primary/10 text-primary px-2 py-0.5 rounded-md text-[9px] font-black">{filteredSessions.length}</span>
            </h3>
            <div className="flex items-center gap-1">
              <select 
                value={statusFilter}
                onChange={(e) => setStatusFilter(e.target.value)}
                className="text-[10px] bg-transparent border-none focus:ring-0 text-gray-500 font-black uppercase tracking-tight cursor-pointer pr-8"
              >
                  ...
                  <div className="flex items-center gap-3 mb-1.5 w-full pr-8">
                    ...
                    <div className="flex flex-col flex-grow overflow-hidden text-left">
                       <span className={`font-black text-sm tracking-tight truncate ${isActive ? 'text-primary' : 'text-gray-900'}`}>
                        {s.customerFull || s.guest_name || 'Guest User'}
                      </span>
        ...
        <div className={`
          ...
          ${!selectedChat && typeof window !== 'undefined' && window.innerWidth < 768 ? 'opacity-0 pointer-events-none translate-x-full' : 'opacity-100 translate-x-0'}
        `}>
          {selectedChat ? (
            <>
              {/* Thread Header */}
              <div className="h-16 px-4 md:px-6 border-b border-gray-100 flex items-center justify-between bg-white shadow-sm z-10 sticky top-0 flex-shrink-0">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  <button 
                    onClick={() => setSelectedChat(null)}
                    className="md:hidden w-9 h-9 rounded-xl bg-gray-50 hover:bg-gray-100 flex items-center justify-center text-gray-500 active:scale-90 transition-transform flex-shrink-0"
                  >
                    <span className="material-symbols-outlined font-black">chevron_left</span>
                  </button>
                  <div className="w-9 h-9 rounded-xl bg-primary text-white flex items-center justify-center font-black text-xs shadow-lg shadow-primary/20 flex-shrink-0">
                  ...
                  <div className="flex flex-col min-w-0 pr-2">
                    <div className="flex items-center gap-1.5 min-w-0">
                      ...
                      {activeChat.status === 'CLOSED' && (
                        <span className="text-[7px] bg-red-50 text-red-500 px-1 py-0.5 rounded font-black uppercase tracking-widest border border-red-100 flex-shrink-0">Closed</span>
                      )}
                    </div>
                    <p className="text-[9px] text-gray-400 font-bold uppercase tracking-widest flex items-center gap-1.5 leading-none">
                      <span className={`w-1.5 h-1.5 rounded-full ${activeChat.status === 'OPEN' ? 'bg-green-500' : 'bg-gray-300'}`} />
                      {activeChat.customer_id ? 'Authenticated' : 'Sessional'}
                    </p>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 flex-shrink-0">
                  <button 
                    ...
                    className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all active:scale-95 ${isThreadSearchOpen ? 'bg-primary text-white shadow-lg shadow-primary/20' : 'hover:bg-gray-50 text-gray-400 hover:text-gray-900'}`}
                  >
                  ...
                  <div className="relative">
                    <button 
                      ...
                      className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all active:scale-95 ${isMoreMenuOpen ? 'bg-gray-100 text-gray-900' : 'hover:bg-gray-50 text-gray-400 hover:text-gray-700'}`}
                    >
                      <span className="material-symbols-outlined text-[18px] font-bold">more_vert</span>
                    </button>
                    {isMoreMenuOpen && (
                      <div className="absolute right-0 mt-3 w-52 bg-white border border-gray-100 rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.15)] z-30 overflow-hidden animate-in fade-in zoom-in-95 duration-200 origin-top-right p-1.5">
                        <button 
              ...
              <div className="flex-1 overflow-y-auto px-4 md:px-8 py-6 space-y-5 bg-[#F9FAFB]/50 scroll-smooth">
                {loadingMessages ? (
                  ...
                  displayMessages.map((m, idx) => {
                    ...
                    <div key={m.id || idx} className={`flex ${m.is_from_admin ? 'justify-end' : 'justify-start'} ${isSequence ? '!mt-1' : ''}`}>
                      <div className="flex flex-col max-w-[88%] sm:max-w-[80%] md:max-w-[70%] lg:max-w-[65%]">
                         {!isSequence && !m.is_from_admin && (
                           <span className="text-[9px] font-black uppercase tracking-widest text-gray-400 mb-1.5 ml-2">{m.sender_name || activeChat.guest_name}</span>
                         )}
                         <div className={`px-4 py-3 text-[14px] font-semibold leading-relaxed shadow-sm transition-all ${
                           m.is_from_admin 
                           ...
                           {m.file_url ? (
                             <div className="flex flex-col gap-3 min-w-[180px]">
                                <div className="flex items-center gap-3 p-1">
                                  <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${m.is_from_admin ? 'bg-white/20' : 'bg-primary/10 text-primary'}`}>
                                    <span className="material-symbols-outlined text-xl">
                                      {m.mime_type?.startsWith('image/') ? 'image' : 'description'}
                                    </span>
                                  </div>
                                  <div className="flex flex-col overflow-hidden">
                                     <span className="text-[12px] font-black truncate max-w-[150px] leading-tight">{m.file_name}</span>
                                     <span className="text-[8px] opacity-70 uppercase tracking-widest font-black mt-0.5">Asset</span>
                                  </div>
                                </div>
                                {m.mime_type?.startsWith('image/') && (
                                   <div className="mt-1 rounded-xl overflow-hidden border border-black/5 bg-gray-50 group relative">
                                      <img src={m.file_url} alt={m.file_name} className="max-h-[250px] w-full object-contain mx-auto transition-transform" />
                                      <a href={m.file_url} target="_blank" rel="noreferrer" className="absolute top-2 right-2 w-8 h-8 bg-black/40 backdrop-blur-md rounded-lg flex items-center justify-center text-white md:opacity-0 group-hover:opacity-100 transition-opacity">
                                ...
                                <a 
                                  ...
                                  className={`py-2.5 px-4 rounded-xl flex items-center justify-center gap-2 text-[10px] font-black uppercase tracking-widest transition-all ${
                                    m.is_from_admin ? 'bg-white/10 hover:bg-white/20 text-white' : 'bg-gray-100 hover:bg-gray-200 text-gray-800'
                                  }`}
                                >
                         ...
                         {!isSequence && (
                           <span className={`text-[8px] text-gray-400 mt-1.5 px-1 font-black uppercase tracking-tighter ${m.is_from_admin ? 'text-right' : 'text-left'}`}>
                ...
                {isUploading && (
                  <div className="flex justify-end animate-in slide-in-from-right-4">
                    <div className="bg-primary/5 px-4 py-2.5 rounded-2xl rounded-tr-sm border border-primary/10 flex items-center gap-3 text-[10px] text-primary font-black uppercase tracking-widest shadow-sm">
              ...
              <div className="p-4 md:p-6 bg-white border-t border-gray-100 flex-shrink-0">
                <form onSubmit={handleSend} className="flex gap-2 items-end max-w-full">
                  <div className="flex-1 bg-gray-50 border-2 border-transparent focus-within:border-primary/20 focus-within:ring-4 focus-within:ring-primary/5 focus-within:bg-white rounded-[1.25rem] transition-all flex items-center overflow-hidden">
                    <button type="button" className="pl-3 pr-1 py-3 text-gray-400 hover:text-primary transition-colors">
                      <span className="material-symbols-outlined text-[20px] font-bold leading-none">sentiment_satisfied</span>
                    </button>
                    <input
                      type="text"
                      value={inputValue}
                      onChange={e => setInputValue(e.target.value)}
                      placeholder={selectedChat.status === 'CLOSED' ? "Thread Locked" : "Compose response..."}
                      disabled={selectedChat.status === 'CLOSED'}
                      className="flex-1 bg-transparent py-3 text-sm font-bold outline-none text-gray-900 placeholder-gray-400 disabled:cursor-not-allowed px-1 min-w-0"
                    />
                    <button 
                      type="button" 
                      onClick={() => fileInputRef.current?.click()}
                      disabled={selectedChat.status === 'CLOSED' || isUploading}
                      className="px-3 py-3 text-gray-400 hover:text-primary transition-colors disabled:opacity-30 flex-shrink-0"
                    >
                      <span className="material-symbols-outlined text-[20px] font-bold leading-none">add_circle</span>
                    </button>
                  ...
                  <button
                    ...
                    className="w-12 h-12 bg-primary hover:bg-primary/90 text-white rounded-2xl flex items-center justify-center disabled:opacity-30 transition-all shadow-lg shadow-primary/20 active:scale-90 flex-shrink-0"
                  >
            ...
            <div className="flex-1 flex flex-col items-center justify-center bg-[#F9FAFB]/50 p-8 text-center animate-in fade-in duration-500">
              <div className="w-20 h-20 bg-primary/5 rounded-[2rem] flex items-center justify-center mb-6 shadow-sm border border-primary/5">
                <span className="material-symbols-outlined text-primary/30 text-[40px] animate-pulse">forum</span>
              </div>
              <h3 className="font-black text-lg text-gray-900 mb-2 tracking-tight uppercase tracking-widest">Support Portal</h3>
              <p className="text-[12px] font-semibold text-gray-400 max-w-xs leading-relaxed">
                Select an active conversation thread from the sidebar to begin communicating with your clients.
              </p>
            </div>
          )}
        </div>
