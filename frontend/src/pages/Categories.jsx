...
export default function Categories() {
  ...
  const handleRequestQuote = (categoryKey) => navigate(`/products?category=${categoryKey}`)
    ...
    <>
      ...
      <section className="bg-surface-container-low py-6 px-8 sticky top-16 z-30 shadow-sm">
        <div className="max-w-screen-2xl mx-auto flex gap-3 overflow-x-auto pb-1 no-scrollbar">
          {CATEGORIES.map((cat) => (
            <button
              key={cat.key}
              onClick={() => { setActiveCategory(cat.key); document.getElementById(cat.key)?.scrollIntoView({ behavior: 'smooth', block: 'start' }) }}
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold whitespace-nowrap transition-all flex-shrink-0 ${activeCategory === cat.key ? 'bg-primary text-white shadow-md' : 'bg-surface-container-lowest text-on-surface-variant hover:bg-secondary-container hover:text-primary'}`}
            >
      ...
      <div className="max-w-screen-2xl mx-auto px-8">
        {CATEGORIES.map((cat, i) => (
          <section key={cat.key} id={cat.key} className={`py-24 ${i < CATEGORIES.length - 1 ? 'border-b border-surface-container' : ''}`}>
            <div className={`grid md:grid-cols-2 gap-16 items-center ${i % 2 !== 0 ? 'md:grid-flow-dense' : ''}`}>
              ...
              <div className={`relative ${i % 2 !== 0 ? 'md:col-start-2' : ''}`}>
                <div className="rounded-2xl overflow-hidden shadow-2xl aspect-[4/3]">
                  <img src={cat.img} alt={cat.label} className="w-full h-full object-cover hover:scale-105 transition-transform duration-700" />
                </div>
              ...
              <div className={i % 2 !== 0 ? 'md:col-start-1 md:row-start-1' : ''}>
                ...
                <h2 className="font-headline font-extrabold text-3xl md:text-4xl text-on-surface mb-6 leading-tight">{cat.label}</h2>
                ...
                {cat.examples?.length > 0 && (
                  <div className="bg-surface-container-low rounded-xl p-5 mb-8">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {cat.examples.map((ex) => (
                        <span key={ex} className="px-3 py-1.5 bg-surface-container-lowest rounded-full text-xs font-medium text-on-surface-variant">{ex}</span>
                      ))}
                    </div>
                  </div>
                )}
                <div className="flex gap-4">
                  <button onClick={() => handleRequestQuote(cat.key)} className="signature-gradient text-white px-8 py-3 rounded-xl font-headline font-bold shadow-lg hover:opacity-90 active:scale-95 transition-all flex items-center gap-2">
                    <span className="material-symbols-outlined text-lg">request_quote</span>
                    Request Quote
                  </button>
                  <Link to={`/products?category=${cat.key}`} className="border border-primary/20 text-primary px-8 py-3 rounded-xl font-headline font-bold hover:bg-primary hover:text-white transition-all flex items-center gap-2">
                    Browse Products <span className="material-symbols-outlined text-lg">arrow_forward</span>
                  </Link>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-screen-xl mx-auto">
          <div className="bg-primary rounded-3xl p-12 md:p-16 relative overflow-hidden">
            ...
            <div className="relative z-10 grid md:grid-cols-2 gap-10 items-center">
              <div>
                <h2 className="font-headline font-extrabold text-4xl text-white mb-4">Can't find what you need?</h2>
                <p className="text-blue-200 text-lg leading-relaxed">Our procurement specialists can source any pharmaceutical product globally. Submit a custom RFQ and we'll find it for you.</p>
              </div>
              <div className="flex flex-col sm:flex-row gap-4 md:justify-end">
                <Link to="/rfq" className="bg-white text-primary px-8 py-4 rounded-xl font-headline font-bold hover:scale-[1.02] transition-all shadow-xl inline-flex items-center gap-2 justify-center">
                  Submit Custom RFQ <span className="material-symbols-outlined">arrow_forward</span>
                </Link>
